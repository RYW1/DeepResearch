...
def get_research_topic(messages: List[AnyMessage]) -> str:
    ...
    """
    if len(messages) == 1:  # 如果是单轮消息
        research_topic = messages[-1].content
    else:
        research_topic = ""
        for message in messages:  # 如果是多轮消息，遍历消息列表
            if isinstance(message, HumanMessage):  # 如果遍历到的消息是用户消息
...
def resolve_urls(urls_to_resolve: List[Any], id: int) -> Dict[str, str]:
    ...
    # 遍历搜索结果列表urls_to_resolve，如果满足if条件，就送到site["url"]中得到值并放入新的列表，全部遍历结束，得到组装完成的新列表urls
