...
class RateLimiter:
    """
    简单的令牌桶速率限制器:一个桶里只有15个令牌，用户把需要发请求把15个取走了，没有令牌了，其他用户就只能等着
    ...
    def __init__(self, max_qps: float = 15.0):
        ...
        self.min_interval = 1.0 / max_qps  # 最小请求间隔（秒）,1/15=0.0667,最少间隔这么长时间可以再次接受请求
    ...
    def acquire(self):
        ...
        Returns:
            float: 实际等待的时间（秒）
        """
        with self.lock:  # 自动获取和释放锁，避免死锁。
            current_time = time.time()
            # 获得当前请求时间和上一次请求时间的时间差，是不是>最小时间间隔
            time_since_last = current_time - self.last_request_time

            # 小于，请求太频繁
            if time_since_last < self.min_interval:
                # 需要等待的时间：最小间隔时间-当前最近一次请求时间
                wait_time = self.min_interval - time_since_last
                logger.debug(f"速率限制：需要等待 {wait_time:.3f} 秒")
                time.sleep(wait_time)
                self.last_request_time = time.time()  # 获取当前时间，覆盖掉__init__中的初始值0
                return wait_time
            else:
...
class Agent:
    ...
    def step(self, **kwargs):
        # 把提示词变量替换为实际值
        # 传入的step_prompt提示词中有变量
        # **kwargs是传入的变量的实际值
        step_prompt = self.prompt_format(self.step_prompt, **kwargs)
        response = ""
        for _ in range(3):
            try:
                response = self(step_prompt)  # 调用大模型
                response = self.post_process(response)  # 对模型输出结果后处理
                break
...
# 大模型返回的报文，需要转换为JSON格式
'''
为什么需要JsonAgent？
大模型虽然返回的也是JSON格式，response.content当中是大模型返回的具体内容，是文本
但是问题拆解需要的是拆成关键词列表，需要进行处理
虽然提示词约束了大模型的输出格式，但是大模型是按照概率输出的，可能100次有2次没有按照要求输出
所以直接在大模型封装的Agent中进行格式转换
'''
class JsonAgent(Agent):
    # 构造函数多了第二个入参 keys，默认值 None
    def __init__(self, model_id="qwen2.5-72b-instruct", keys=None):
        super().__init__(model_id)
        self.keys = keys

    # 重写父类的post_process函数
    # JsonAgent.post_process 方法中，keys 参数用于接收一个 Pydantic 模型类 （如 SearchQueryList 或 Reflection ），实现对 LLM 输出的 结构化校验和类型转换
    # 通过self.keys(**result)，将解析的JSON字典解包传入模型构造函数
    ''' Pydantic 自动进行字段验证和类型转换：大模型输出内容通过Pydantic进行类型转换 '''
    def post_process(self, response):
...
class MCPAgent(Agent):
    """
        继承顶层Agent骨架，重写step方法：把调用OpenAI LLM改为调用阿里云MCP服务
        MCP：阿里云模型上下文协议，项目的联网搜索工具托管在MCP平台
        保留父类：prompt模板渲染、重试机制、日志打印能力
    """
    def step(self, **kwargs):
        # 渲染prompt模板，捕获模板渲染异常
        try:
    ...
    def post_process(self, response):
        # 基础MCP响应解析：HTTP 200状态则解析内层JSON文本
...
"""
继承MCPAgent，专门实现网页检索能力:使用MCP服务进行搜索，没有使用大模型

新增：前置限流 + 差异化退避重试 + 多层兼容的响应解析 + 错误精细化分类
"""
class WebSearchAgent(MCPAgent):
    def step(self, prompt, **kwargs):
        ...
        '''获取速率限制器
            例如：阿里云的MCP服务速率限制为15qps，也就是说一秒只能接受15个用户请求
            如果同时有好多用户用我们这个网站，其拆分的关键词加起来超过15，就会报错
            或者单用户一次请求太复杂，需要拆分>15个关键词，都要进行并行的网络搜索，就会报错
        '''
        # 获取全局唯一限流器实例
        ...
        for attempt in range(3):
            try:
                # 在发送请求前进行速率限制检查，尝试获取令牌
                wait_time = rate_limiter.acquire()
                if wait_time > 0:
                    logger.debug(f"速率限制等待: {wait_time:.3f}秒")

                # 如果拿到令牌，可以进行MCP请求
                # 在阿里云官网申请MCP服务时，该服务有两个参数：query:搜索关键词；count:搜索和返回的搜索词条个数（通过kwargs传入）
                response = Application.call(
                    api_key=api_key,
                    app_id=app_id,
                    prompt = step_prompt,
                ...
                # ========= 差异化重试等待策略 =========
                # 如果是429错误，增加等待时间后重试
                # 出现了网络错误，常用的避退算法（backoff）
                # 线性避退算法：wait_time = 5 * (attempt + 1)
                # 指数避退算法：wait_time = 5 * 2^attempt
                '''工程上，当请求第三方服务，触发对方的限流机制的时候，要采用重试+指数避退的方式确保系统服务，避免报错'''
                if "429" in error_msg:
    ...
    """
    重度封装：兼容多种返回结构、区分各类HTTP错误码、规整统一的网页数据格式
    目的：无论MCP接口结构微调，上层永远拿到固定格式[{title,url,snippet}]数组
    """
    def post_process(self, response):
        if response is None:
        ...
        if response.status_code == 200:
            try:
                ...
                # 尝试不同的路径获取pages数据，兼容三种不同的返回数据路径
                pages = None
                ...
                # 提取需要的字段：只保留业务需要的三个字段，精简LLM上下文长度
                processed_pages = []
                for page in pages:
