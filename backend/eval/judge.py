...
class Judge:
    ...
    def _call(self, prompt: str) -> dict[str, Any]:
        ...
        for attempt in range(3):
            try:
                raw = agent(prompt)  # raw 是模型的原始输出
                last_raw = raw
                # 从字符串中提取 JSON 字符串
                json_str = Post.extract_pattern(raw, pattern="json")  # 从 raw 中提取 JSON 字符串,Post.extract_pattern 是一个工具函数,用于从字符串中提取模式匹配的子字符串
                # 把 JSON 字符串转换为 Python 字典
                result = json.loads(json_str)  # 解析 JSON 字符串为 Python 字典,loads 是 JSON 模块的函数,用于将 JSON 字符串转换为 Python 对象
