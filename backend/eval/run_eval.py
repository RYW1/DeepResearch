...
# 加载测试集，传入测试集文件名称，默认使用 test_set.json
...
def main():
    ...
    # 应用 CLI 覆盖参数，覆盖测试集中的默认值
    # 如果指定了 initial_queries 或 max_loops，则覆盖所有主题的默认值
    ...
    # 初始化评估器。judge_model_id 是 Judge LLM 的模型 ID
    evaluator = Evaluator(judge_model_id=args.judge_model)
    # 初始化评估报告。timestamp 是评估时间戳
    ...
    if args.mode in ("e2e", "all"):
        ...
        print("  运行端到端评估...")
        print("=" * 60)
        # 调用evaluator.run_e2e函数运行端到端评估，返回结果
