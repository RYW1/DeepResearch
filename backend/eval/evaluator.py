...
class Evaluator:
    ...
    def run_e2e(self, topics: list[TopicCfg]) -> list[E2EResult]:
        ...
        for i, cfg in enumerate(topics):
            ...
            try:
                # 调用_invoke_agent函数（其中调用_invoke_agent_with_feedback函数执行图的全流程，返回完整字典），提取report和sources用result接收
                result = self._invoke_agent(cfg)
                if result.error:
                    results.append(result)
                    continue

                # 调用judge.evaluate_report函数对最终报告进行评分
                # research_topic 是研究主题
                # search_sources 是搜索到的来源
                # report 是最终报告
    ...
    def _invoke_agent_with_feedback(
        ...
        if cfg.user_feedback is None:  
            # 如果没有指定用户反馈，则直接确认计划：图只中断1次
            if capture:
                capture.patch()
            try:
                # 第二次调用图，传入计划plan和计划确认状态plan_status：图执行到await等待用户输入时就不等了，直接继续执行后续节点
                phase2_state = graph.invoke(
                    {
                        "messages": [
                            HumanMessage(content=cfg.topic),
                            *(phase1_state.get("plan_messages", [])),
                            HumanMessage(content="需求确认"),  # 模拟用户确认计划
                        ],
                        "plan": plan_a,
            ...
            # ---- 阶段 2：发送用户反馈 ----
            # 第二次调用图，传入用户反馈、旧计划plan_a、计划确认状态plan_status，进入生成计划节点后不等待，传入用户输入，进入evaluate_plan进行用户意图识别
            phase2_state = graph.invoke(
                {
                    "messages": [
                        HumanMessage(content=cfg.topic),
            ...
            # LLM接收用户反馈 判定需要重新规划计划，进入"replan"节点，将状态改为"unconfirmed"
            # replan节点执行完，根据图流程跳转到generate_plan节点，重新生成计划，现在又需要用户确认计划，图中断
            if plan_status_after_p2 == "unconfirmed":
                actual_behavior = "replan_then_proceed"  # 更新实际行为（评估状态：actual_behavior）为replan_then_proceed
                plan_b = phase2_state.get("plan", "")  # 获取重新生成的计划，如果没有则为空字符串
                # ---- 阶段 3：确认重新计划后的结果（研究） ----
                if capture:
                    capture.patch()
                try:
                    # 第三次调用图，如果这次用户输入需求确认，就可以顺利的进入到后续节点，执行整个图流程
                    phase2_state = graph.invoke(
                        {
                            "messages": [
                                HumanMessage(content=cfg.topic),
        ...
        return {
            "plan_a": plan_a,  # 初始计划
            "plan_b": plan_b,  # 重新生成的计划
            "actual_behavior": actual_behavior,  # 实际行为（评估状态：actual_behavior）
            "report": report,  # 最终报告
            "sources": sources,  # 搜索内容的来源
            "phase1_state": phase1_state,  # 第一次调用图的状态
            "phase2_state": phase2_state,  # 第二次调用图的状态
        }
    ...
    def _invoke_agent(self, cfg: TopicCfg) -> E2EResult:
        ...
        try:
            ...
            return E2EResult(  # _invoke_agent函数把返回的结果中的report和sources包装在E2EResult返回
                topic=cfg.topic,
                report=result["report"],
    ...
    def _eval_one_topic_components(self, cfg: TopicCfg) -> ComponentResult:
        ...
        plan_a = agent_result["plan_a"]  # 初始计划
        plan_b = agent_result["plan_b"]  # 重新生成的计划
        actual_behavior = agent_result["actual_behavior"]  # 实际行为（评估状态：actual_behavior）
        phase2 = agent_result["phase2_state"]  # 第二次调用图的状态，包含最终报告、搜索查询、摘要等信息
